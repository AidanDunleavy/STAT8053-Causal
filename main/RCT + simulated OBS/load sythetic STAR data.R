...
data <- readRDS("lib/synthetic_star_data12345.rds")
data
...
combined_data <- data %>%
  transmute(
    # id = student_id,
    V = as.numeric(as.factor(study)) - 1L,
    X1 = gender,
    X2 = fct_collapse(
      race,
...
compute_gformula(combined_data)

compute_aipsw(combined_data,p =0) # p was unused

compute_ipsw(combined_data)
...
class(combined_data$X2)

table(combined_data$X2)

levels(combined_data$X2)
...
combined_data$study
which(is.na(combined_data$study))


table(temp[temp$V == 1 & temp$A == 1, !names(temp) %in% c("V", "A")]$X2)
table(temp[temp$V == 1 & temp$A == 0, !names(temp) %in% c("V", "A")]$X2)

levels(temp[temp$V == 1 & temp$A == 1, !names(temp) %in% c("V", "A")]$X2)
table(temp[temp$V == 0, !names(temp) %in% c("V", "A")]$X2)
